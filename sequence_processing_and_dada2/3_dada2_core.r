...
workdir = args[1]
seqDir = file.path(workdir, "qual_filter")
...
qualFs <- sort(list.files(seqDir, pattern = "_R1_001.fastq.gz", full.names = TRUE))
qualRs <- sort(list.files(seqDir, pattern = "_R2_001.fastq.gz", full.names = TRUE))
...
errF <- learnErrors(qualFs, multithread = TRUE)
errR <- learnErrors(qualRs, multithread = TRUE)
...
derepFs <- derepFastq(qualFs, verbose = TRUE)
derepRs <- derepFastq(qualRs, verbose = TRUE)

get.sample.name <- function(fname) strsplit(basename(fname), "_L00")[[1]][1]
sample.names <- unname(sapply(qualFs, get.sample.name))
